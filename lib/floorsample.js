...
var iBot = function Constructor(settings) {
    ...
    this.settings.name = this.settings.name || 'innovate';
    this.user = null;
};
...
iBot.prototype._smartresponse = function (originalMessage) {
    ...
    http.get(roomurl, function(response) {
        if (originalMessage.username !== 'slackbot') {
            ...
            self.postMessageToChannel(channel.name, 'V', {as_user: true})
        }
    })
        } else { 
            self.filesupload(n, filename, channel.name);
            self.postMessageToChannel(channel.name, "Sorry! I don't have a map for that room:" , {as_user: true})
        }
};
