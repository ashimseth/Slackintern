...
SethuBot.prototype._onMessage = function (message) {
    ...
    { 
        ...
        this._smartestresponse(message);
    }
};
...
SethuBot.prototype._smartestresponse = function (originalMessage) {
    var self = this;
    var channel = self._getChannelById(originalMessage.channel);
    var newMessage = originalMessage.text.split('>:')[1];
    var a = newMessage.substring(1,2);
    var b = newMessage.substring(2);
    var roomurl = urlName1 + a + urlName2 + b + sample;
    ...
    http.get(roomurl, function(response) {
        if (originalMessage.username !== 'slackbot') {
            ...
            self.postMessageToChannel(channel.name, 'VBVCB', {as_user: true})
    }
    })
};
